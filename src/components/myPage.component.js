...
function TripBooking() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setUserId(userId);
        const response = await axios.get(`https://travel.onrender.com/api/v1/destination/${destinationId}/user/${userId}`);
  ...
  const updateUserInfo = async (userInfo) => {
    try { 
      const response = await axios.patch(`https://travel.onrender.com/api/v1/destination/${destinationId}/user/${userId}`, userInfo);
  ...
  if (updatedUser) {
    try {
        ...
        await axios.post(`https://travel.onrender.com/api/v1/myFlights/${userId}`, {
