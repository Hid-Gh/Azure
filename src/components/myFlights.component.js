...
function MyFlights() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        setUserId(userId);
        const response = await axios.get(`https://travel.onrender.com/api/v1/myFlights/${userId}`);
  ...
  const handleDeleteAll = async () => {
    try {
      const response = await axios.delete(`https://travel.onrender.com/api/v1/myFlights/${userId}`);
